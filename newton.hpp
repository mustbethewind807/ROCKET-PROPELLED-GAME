...
namespace newton {
  ...
  typedef struct
  {
    glm::vec2 pos;                        // The current position of the Entity.
    glm::vec2 prevPos;                    // The position of the Entity on the previous update cycle.
    glm::vec2 vel;                        // The current velocity of the Entity.
    glm::vec2 acc;                        // The force that will be applied on the next update cycle.
  ...
  typedef struct
  {
    glm::vec2 pos;          // The current position of the Camera.
  ...
  typedef struct
  {
    ...
    glm::vec2 gravity;                 // A constant force applied onto all Entities inside the Engine each update cycle.
  } Engine;
  ...
  // Entity functions
  void applyForce(Entity, glm::vec2);
